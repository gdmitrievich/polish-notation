...
void MenuDataContainer::regenerateFunction() {
    ...
    if (!pn::parser::trySetTokenQueueFromStr(qTokens, funcLineWithoutSpaces))
...
void MenuDataContainer::regenerateEditableOption(int idx) {
    switch (idx) {
        ...
        case 2:
            ...
            break;
        case 3:
            editableOptions[idx].second =
                "[" + std::to_string(fInfo_.domain.first) + ", " +
                std::to_string(fInfo_.domain.second) + "]";
            break;
        case 4:
            editableOptions[idx].second =
                "[" + std::to_string(fInfo_.codomain.first) + ", " +
                std::to_string(fInfo_.codomain.second) + "]";
            break;
        case 5:
            editableOptions[idx].second =
                "(" + std::to_string(fInfo_.centerOfCoordinates.first) + ", " +
                std::to_string(fInfo_.centerOfCoordinates.second) + ")";
            break;
    }
}
...
void MenuDataContainer::processPressedKeyInSelectMode(char pressedKey) {
    int v =
        returnValueIfOneOfTheKeyPressed(-1, pressedKey, {'w', 'd', 'k', 'l'});
    if (v == 0)
        v = returnValueIfOneOfTheKeyPressed(1, pressedKey,
                                            {'s', 'a', 'j', 'h'});
    if (v == 0)
        return;
...
bool MenuDataContainer::processPressedKeyInEditMode(char pressedKey) {
    ...
    if (arrowPos_ == 1 || arrowPos_ == 2) {
        ...
        if (v == 0)
            v = returnValueIfOneOfTheKeyPressed(-1, pressedKey, {'a', 'h'});
        ...
        if (v == 0)
            v = returnValueIfOneOfTheKeyPressed(-1, pressedKey, {'s', 'j'});
        if (v == 0)
            return false;
...
int MenuDataContainer::returnValueIfOneOfTheKeyPressed(
