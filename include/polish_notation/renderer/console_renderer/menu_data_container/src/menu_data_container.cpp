...
MenuDataContainer::MenuDataContainer() : funcStr_(), funcPostfixTokenQueue_(), fInfo_(), field_(), actionType_(ActionType::Select), arrowPos_() {
	try {
...
void MenuDataContainer::regenerateFunction() {
	...
	try {
		...
		funcPostfixTokenQueue_ = pn::shunting_yard_alg::convertInfixTokenQueueToPostfix(qTokens);
	} catch (const ::std::exception& e) {
		pn_uemp::printErrMess(e);
		exit(EXIT_FAILURE);
	}
